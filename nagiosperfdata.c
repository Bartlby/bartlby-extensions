		...
		if(token != NULL) {
			...
			if(token != NULL) {
				...
				if(fp != NULL) {
					...
					perflog_mem=(strlen(cfg_output_format)+40+strlen(svc->service_name)+strlen(PROGNAME)+20+strlen(svc->srv->server_name)+strlen(svc->service_name)+40+strlen(svc->current_output));
